...
class TestGamesDefs(unittest.TestCase):
    ...
    def test_get_game_by_name_sucess(self):
        ...
        mock_find_one.return_value = {"name": "Game", "year": 2021}

        response = get_game_by_name('Game')

        mock_find_one.assert_called_once_with({'name': 'Game'}, {"_id": 0, "name": 1, "year": 1})

        self.assertEqual(response.status_code, 200)
        self.assertIn('Game found', response.get_json()['message'])

    @patch('utils.games_defs.verify_name')
    @patch('utils.games_defs.verify_year')
    def test_post_games(self, mock_verify_year, mock_verify_name):
    ...
    @patch('utils.games_defs.verify_id')
